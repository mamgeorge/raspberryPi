...
def sampleLoad(count):
	...
	while any( ctr < spaces for ctr in anyProgress):
		...
		print( u"\u001b[1000D", end="" ) # Move left
		print( u"\u001b[" + str(count) + "A", end="" ) # Move up
		for progress in anyProgress:
			width = progress / 4
			chars = Style.BRIGHT + Fore.CYAN + "\u2584" * int(width) + Style.RESET_ALL
			blnks = " " * ( 25 - int(width) )
...
timer = str( datetime.datetime.now( ) )
print( Style.BRIGHT + Fore.GREEN + "INIT: " + timer + Style.RESET_ALL, end="" )
sampleLoad(4)
